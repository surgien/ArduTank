...
namespace ControlUnit.Controller.Core.Services
{
    ...
    public class UARTService
    {
        ...
        public async Task UART_Transmit(string user_input, int inputlength)
        {
            ...
            try
            {
                ...
                var x = await tx_characteristic.WriteValueAsync(data.AsBuffer(), GattWriteOption.WriteWithoutResponse);
            }
            catch (Exception)
            {
                // ignore exception
    ...
    public sealed class UART_Data_Converter
    {
        ...
        {
            return transmit_this.ToCharArray().Select(c => Convert.ToByte(c)).ToArray();
        ...
        {
            return new string(recived_this.Select(c => (char)c).ToArray());
